...
import { Transaction, type TxnStatus } from "@/models";
...
export default function TransactionsPageClient({ params, searchParams }: { params?: Record<string, string>; searchParams?: Record<string, string> }) {
  ...
  await dbConnect();
  const filter = status && ["pending", "approved", "rejected"].includes(status) ? { status: status as TxnStatus } : {};
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead className="text-xs uppercase text-slate-500">
              <tr><th className="pb-2">User</th><th className="pb-2">Type</th><th className="pb-2">Provider</th><th className="pb-2">Amount</th><th className="pb-2">Account / TID</th><th className="pb-2">Status</th><th className="pb-2">Time</th><th className="pb-2">Action</th></tr>
            </thead>
                ...
                return (
                  <tr key={id}>
                    ...
                    <td className="py-2.5 text-xs text-slate-300">
                      {t.type === "deposit" ? (
                        <>
                          <div>To: {t.paymentAccountId?.accountTitle ?? "-"} ({t.paymentAccountId?.accountNumber ?? "-"})</div>
                          <div>From: {t.senderNumber}</div>
                          <div className="font-mono text-yellow-400">TID: {t.referenceId}</div>
                        </>
                      ) : (
                        <div>Send to: <span className="font-mono text-yellow-400">{t.senderNumber}</span></div>
                      )}
                    </td>
                    ...
                    <td className="py-2.5">
                      {t.status === "pending" && (
                        <div className="flex gap-1.5">
                          <form action={processTransactionAction.bind(null, id, "approved", undefined)}>
                          ...
                          <form action={processTransactionAction.bind(null, id, "rejected", "Rejected by admin")}>
