...
export function InstantPayForm({ kind, min, max, label, hasPin }: { kind: "deposit" | "withdraw"; min: number; max: number; label: string; hasPin: boolean }) {
  const { isUr } = useI18n();
  ...
  const go = async () => {
    ...
    const r = await localApi("/api/gateway", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "create", kind, provider, amount, accountNumber: acc, pin }) });
