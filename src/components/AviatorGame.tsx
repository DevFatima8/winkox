    ...
    <div className="relative overflow-hidden rounded-2xl text-[13px] text-[#bbbfc5]" style={{ background: T.bg }}>
      ...
      <div className="grid lg:grid-cols-[260px_1fr] xl:grid-cols-[300px_1fr]">
        ...
        <section className="order-1 flex flex-col lg:order-2">
          ...
          {/* bet panels */}
          <div className={`grid gap-2 p-2 ${slots === 3 ? "md:grid-cols-3" : "md:grid-cols-2"}`} style={{ background: T.panel2 }}>
            {Array.from({ length: slots }, (_, i) => {
              ...
              return (
                <div key={i} className="rounded-xl p-2.5" style={{ background: T.panel, border: `1px solid ${border}` }}>
                  <div className="mb-2 flex justify-center"><div className="flex rounded-full p-0.5 text-[11px] font-semibold" style={{ background: T.panel2 }}><button onClick={() => setPanel(i, { tab: "bet" })} className={`rounded-full px-4 py-0.5 ${p.tab === "bet" ? "bg-[#2c2d30] text-white" : "text-slate-400"}`}>Bet</button><button onClick={() => setPanel(i, { tab: "auto" })} className={`rounded-full px-4 py-0.5 ${p.tab === "auto" ? "bg-[#2c2d30] text-white" : "text-slate-400"}`}>Auto</button></div></div>
                  <div className="grid grid-cols-[1fr_1.2fr] gap-2">
                    <div className={locked ? "pointer-events-none opacity-60" : ""}>
                      ...
                      <div className="mt-1.5 grid grid-cols-2 gap-1">{[100, 200, 500, 1000].map((v) => <button key={v} onClick={() => setAmt(v)} className="rounded-full py-[3px] text-[11px] font-semibold text-slate-400 hover:text-white" style={{ background: T.panel2, border: `1px solid ${T.line}` }}>{v.toFixed(2)}</button>)}</div>
                    </div>
                    <div className="flex min-h-[74px]">
                      ...
                      {mode === "cashout" && <button data-action="cashout" disabled={p.busy} onClick={() => cashOut(i)} className="flex w-full flex-col items-center justify-center rounded-2xl bg-[#d07206] text-white shadow-[inset_0_-3px_0_rgba(0,0,0,.25)] transition hover:bg-[#f18a12] disabled:opacity-60"><span className="text-lg font-semibold uppercase leading-none">Cash out</span><span className="mt-1 text-base font-bold">{fmt2(potential)} <span className="text-[10px]">PKR</span></span></button>}
                    </div>
                  </div>
                  {mine && mine.outcome !== "pending" && <div className={`mt-2 rounded-lg px-2 py-1 text-center text-[11px] font-semibold ${mine.outcome === "win" ? "bg-[#123717] text-[#4ade80]" : "bg-[#3a0b12] text-red-300"}`}>{mine.outcome === "win" ? `Cashed out ${fmt2(mine.win)} PKR` : `Lost ${fmt2(mine.bet)} PKR`}</div>}
                  {p.tab === "auto" && (
                    <div className="mt-2 flex flex-wrap items-center justify-between gap-2 rounded-lg px-2 py-1.5 text-[11px]" style={{ background: T.panel2, border: `1px solid ${T.line}` }}>
                      <label className="flex items-center gap-2"><span className="text-slate-300">Auto Bet</span><Toggle on={p.autoBet} onChange={(v) => setPanel(i, { autoBet: v })} /></label>
                      <label className="flex items-center gap-2"><span className="text-slate-300">Auto Cash Out</span><Toggle on={p.autoCash} onChange={(v) => setPanel(i, { autoCash: v })} /><span className="flex items-center rounded-full px-2" style={{ background: T.panel, border: `1px solid ${T.line}` }}><input value={p.autoCashAt} disabled={!p.autoCash} onChange={(e) => setPanel(i, { autoCashAt: e.target.value })} onBlur={() => setPanel(i, { autoCashAt: Math.max(1.01, Number(p.autoCashAt) || 1.01).toFixed(2) })} className="w-12 bg-transparent py-0.5 text-center font-bold text-white outline-none disabled:opacity-40" /><span className="text-slate-500">x</span></span></label>
                    </div>
                  )}
                </div>
              );
