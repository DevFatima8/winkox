...
const GAMES = [
  ...
  { slug: "chicken-dash", name: "Chicken Dash", icon: "🐤", category: "original", description: "Busy highway cross karein — Dash boost aur Bonus Bag ke saath 19,659x tak!" },
  { slug: "plinko", name: "Plinko", icon: "🔴", category: "original", description: "Ball girao, pegs se takra kar multiplier bucket mein — 1000x tak!" },
  { slug: "limbo", name: "Limbo", icon: "🎯", category: "original", description: "Target multiplier set karein — 1,000,000x tak instant result!" },
...
export async function ensurePaymentAccounts() {
  if ((await PaymentAccount.countDocuments()) === 0) {
    await PaymentAccount.insertMany([{ provider: "jazzcash", accountTitle: "WinX555 Official", accountNumber: "03001112233", isActive: true }, { provider: "easypaisa", accountTitle: "WinX555 Official", accountNumber: "03334445566", isActive: true }]);
  }
}
